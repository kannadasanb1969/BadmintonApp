...
const { canShowMatchMutation, getCompletionBlockedReason, getMatchCompletionStatus, isMatchCompletionEligible, validWinningPoints } = await import('../node_modules/.cache/match-lifecycle.mjs')
...
for (const [a, b, target, eligible] of cases) assert.equal(isMatchCompletionEligible(a, b, target), eligible, `${a}-${b}, target ${target}`)

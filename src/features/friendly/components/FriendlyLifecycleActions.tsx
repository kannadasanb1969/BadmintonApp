...
export const FriendlyLifecycleActions = ({ match }: { match: FriendlyMatch }) => {
  ...
  return <section className="rounded-2xl border bg-white p-5 shadow-sm sm:p-6"><h2 className="text-xl font-black">Lifecycle</h2>{match.status === 'CLEANUP_PENDING' && <div role="status" className="mt-3 rounded-xl bg-amber-50 p-3"><p className="font-black">Friendly Match closed</p><p className="text-sm">Cleanup pending</p></div>}{message && <p role="status" className="mt-3 rounded-xl bg-emerald-50 p-3 text-emerald-800">{message}</p>}{error && <p role="alert" className="mt-3 text-sm font-semibold text-rose-700">{error}</p>}{action === 'CLOSE' && <button disabled={busy} onClick={() => setConfirming('CLOSE')} className="mt-4 min-h-11 w-full rounded-xl bg-slate-900 px-4 py-3 font-bold text-white disabled:opacity-50 sm:w-auto">{close.isPending ? 'Closing...' : 'Close Friendly Match'}</button>}{action === 'CLEANUP' && <button disabled={busy} onClick={() => setConfirming('CLEANUP')} className="mt-4 min-h-11 w-full rounded-xl bg-rose-700 px-4 py-3 font-bold text-white disabled:opacity-50 sm:w-auto">{cleanup.isPending ? 'Cleaning up...' : 'Cleanup Friendly Data'}</button>}{confirming && <FriendlyLifecycleConfirmDialog kind={confirming} busy={busy} onCancel={() => setConfirming(null)} onConfirm={() => void submit()} />}</section>
}

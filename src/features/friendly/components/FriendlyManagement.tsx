...
export const JoinRequestsContent = ({ rows, actingId, onApprove, onReject }: { rows: FriendlyJoinRequestRow[]; actingId?: string; onApprove: (id: string) => void; onReject: (id: string) => void }) => {
  ...
  return <div className="grid gap-3">{rows.map(row => <article key={row.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0"><p className="break-words font-black text-slate-900">{row.full_name}</p><p className="mt-1 text-xs text-slate-500">{row.player_code} · {readableDate(row.created_at)}</p></div><span className="mobile-status-badge bg-white text-slate-700 shadow-sm">{row.status}</span></div>{row.status === 'PENDING' && <div className="mt-3 flex flex-col gap-2 min-[360px]:flex-row"><button disabled={Boolean(actingId)} onClick={() => onApprove(row.id)} className="min-h-11 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{actingId === row.id ? 'Working...' : 'Approve'}</button><button disabled={Boolean(actingId)} onClick={() => onReject(row.id)} className="min-h-11 rounded-xl border border-rose-200 bg-white px-4 py-2 text-sm font-bold text-rose-700 disabled:opacity-50">Reject</button></div>}</article>)}</div>
}
...
export const ParticipantsContent = ({ rows }: { rows: FriendlyParticipant[] }) => !rows.length
  ...
  : <div className="grid gap-3 sm:grid-cols-2">{rows.map(row => <article key={row.id} className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50/70 p-4"><p className="break-words font-black text-slate-900">{row.full_name}</p><p className="mt-1 text-xs font-semibold text-emerald-700">{row.player_code}</p><p className="mt-3 text-xs text-slate-500">Joined {readableDate(row.created_at)}</p></article>)}</div>

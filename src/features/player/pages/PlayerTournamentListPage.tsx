...
const PlayerTournamentListPage = () => {
  ...
  const [status, setStatus] = useState<TournamentListStatus>('OPEN')
  ...
  const statusColor = { OPEN: 'bg-emerald-50 text-emerald-700', CLOSED: 'bg-rose-50 text-rose-700', COMPLETED: 'bg-blue-50 text-blue-700', ALL: 'bg-slate-50 text-slate-700' }
  const emptyMessage = { OPEN: 'No open registration tournaments found.', CLOSED: 'No registration-closed tournaments found.', COMPLETED: 'No completed tournaments found.', ALL: 'No matching tournaments found.' }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center">
          ...
          <select aria-label="Tournament status" value={status} onChange={event => setStatus(event.target.value as TournamentListStatus)} className={`min-w-0 w-full rounded-xl border border-slate-200 px-2 py-3 text-xs font-medium sm:px-4 sm:text-sm ${statusColor[status]}`}>
